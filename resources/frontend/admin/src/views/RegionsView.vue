        ...
        <div class="col-12 mt-5 text-center" v-if="!show">
            ...
            <div class="d-flex col-12 flex-row text-center justify-content-center">
                <div class="m-3 p-3 col-6">
                    ...
                    <div class="mt-5">Edge</div>
                    <select v-model="edge_id" class="form-select m-3 mb-0  p-3" aria-label="Edge">
        ...
        <div class="d-flex col-12 text-center justify-content-center">
            <div class="col-6">
                <table class="table">
                    ...
                    <template v-for="(region, key) of regionsArr" v-bind:key="key">
                        <tr :class="!isShow(region.id) ? '' : 'd-none'">
                            <td>{{ key + 1 }}</td>
                            <td>{{ region.name }}</td>
                            <td>{{ region.edge.name }}</td>
                            <td>
                                <a class="text-info" @click.prevent="changeId(region.id, region.name, region.edge_id)">
                            ...
                            <td>
                                <a class="text-danger" @click.prevent="del(region.id)">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                        ...
                        <tr :class="isShow(region.id) ? '' : 'd-none'">
                            ...
                            <td>
                                <select v-model="edge_id" class="form-select" aria-label="Edge">
                                    <option disabled value="">Edge</option>
                                    <option v-for="(e, k) in edges" :value="e.id" :key="k">
                                        {{ e.name }}
                                    </option>
                            ...
                            <td>
                                <a class="text-info" @click="update(region.id)">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                ...
                <div class="col-12 text-center text-dark d-flex col-12 text-center justify-content-center">
                    <pagination :pagination="regions" :callback="getResults" :size="'small'">
...
export default {
    ...
    data() {
        return {
            ...
            errE: '',
            show: true,
            url: this.$store.state.URL+'/api/admin/regions',
        }
    },
    methods: {
        add() {
